...
public class TransactionsController {
  ...
  public ResponseEntity<?> performTransaction(
    ...
    Account account = accountService.findAccountByCardNumber(
      cardNumberOnRequest
    );

    if (
      !creditCardRepository.existsCreditCardByCardNumber(cardNumberOnRequest) ||
      !creditCardRepository.existsCreditCardBySecurityNumber(
        securityNumberOnRequest
      ) ||
      !creditCardRepository.existsCreditCardByCardOwner(cardOwnerOnRequest)
    ) {
      ...
      transactionsService.performTransaction(cardNumberOnRequest, amount);
      return new ResponseEntity<>(
